...
class SimpleClipLinear(nn.Module):
    def __init__(self, in_features, out_features, outlier_idx, bias=True, quantize_output=False, nbits=8):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
    ...
    def to(self, *args, **kwargs):
        super(SimpleClipLinear, self).to(*args, **kwargs)
        self.weight = self.weight.to(*args, **kwargs)
        self.B = self.B.to(*args, **kwargs)
        if self.bias is not None:
            self.bias = self.bias.to(*args, **kwargs)
    ...
    def forward(self, x):
        if self.outlier_features != 0:
            ...
            normal_idx = [i for i in range(in_features) if i not in self.outlier_idx]
            x_1 = x[..., normal_idx]
            x_2 = x[..., self.outlier_idx]
            q_x_1 = self.Qa(x_1, self.nbits)
            y_1 = torch.functional.F.linear(q_x_1, self.weight[..., normal_idx], self.bias)
            y_2 = torch.functional.F.linear(x_2, self.B)
            return self.Qo(y_1, self.nbits) + y_2
        else:
            q_x = self.Qa(x, self.nbits)
            y = torch.functional.F.linear(q_x, self.weight, self.bias)
            q_y = self.Qo(y, self.nbits)
